...
Using MP14 because the data used in this analysis is from 2015. 
...
```
Make it easier when you want to use tmap -> tmap will always refer to EPSG code -> change to 3414
...
```
```{r}
mlr.p<- ggcoefstats(condo.mlr1, sort = "descending")
mlr.p
```
...
bw.gwr will calculate bandwidth, put the formula there and tell them the calculation method. GTWR can be calibrated for temporal. 
...
adaptive vs fixed distance is defined under the "adaptive" portion. Ask if your data is in latlong format or not. if latlong, you cannot calculate proximity. if you tell model its lat long, internally it will transform the latlong for you. 
...
gwr.adaptive
```
Checking out adaptive vs fixed, we can see that fixed has a higher r square value vs adaptive.Distance 
